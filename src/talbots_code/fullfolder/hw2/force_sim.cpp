	...
	while (fSimulationRunning) { //automatically set to false when simulation is quit
		...
		switch (enum_problem_part) {
			...
			case PART3:
			    ...
			    redis_client.getEigenMatrixDerivedString("position", ee_des_pos);
			    redis_client.getEigenMatrixDerivedString("rotation", ee_des_rotation);
			    ee_des_pos = ee_des_pos / 10;

			    // ee_des_rotation << 0, 0, -1,
			    // 					0, 1, 0,
			    // 					1, 0, 0;

			    ee_des_pos(0) = ee_des_pos(0) + 0.0;
			    ee_des_pos(1) = ee_des_pos(1) + 0.5;
			    ee_des_pos(2) = ee_des_pos(2) + 0.1;

			    cout<<ee_des_pos<< endl<<endl;
	 ...
	 //    }
        ...
        //cout<< sensed_force << ", " <<sensed_moment << endl <<endl;
   		...
   		redis_client.setEigenMatrixDerivedString("force", sensed_force);
   		redis_client.setEigenMatrixDerivedString("moment", sensed_moment);
