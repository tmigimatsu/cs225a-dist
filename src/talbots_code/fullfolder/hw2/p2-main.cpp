...
void control(Model::ModelInterface* robot, Simulation::SimulationInterface* sim) {
	...
	while (fSimulationRunning) { //automatically set to false when simulation is quit
		...
		p = Jbar.transpose() * g;	
		tau = J0.transpose() * (L * (Eplus*ee_acceleration -kpx * (posandrot) - kvx * (vandw - Eplus * xd) ) + p) + (Eigen::MatrixXd::Identity(6,6) - J0.transpose() * Jbar.transpose())*robot->_M * (-kvj * robot->_dq);
