...
static RedisClient *g_redis = nullptr;

void stop(int) {
	g_runloop = false;
	if (g_redis != nullptr)
		g_redis->set(Puma::KEY_CONTROL_MODE, "BREAK");
...
int main(int argc, char** argv) {
	...
	redis.set(Puma::KEY_CONTROL_MODE, "JHOLD");
	...
	redis.mset({
		...
		{Puma::KEY_COMMAND_DATA, RedisClient::encodeEigenMatrix(q_des)},
	...
	while (g_runloop) {
		...
		// Send command
		x_des << ee_pos_des, ee_ori_des.w(), ee_ori_des.x(), ee_ori_des.y(), ee_ori_des.z();
		redis.setEigenMatrix(Puma::KEY_COMMAND_DATA, x_des);

		// Send desired position for visualization
		redis.setEigenMatrix(Puma::KEY_EE_POS + "_des", ee_pos_des);

		controller_counter++;
	}

	redis.set(Puma::KEY_CONTROL_MODE, "BREAK");
