...
void PumaRedisDriver::eigenVectorFromBuffer(Eigen::VectorXd& vector) {
	assert(vector.size() < kDof || vector.size() < kTaskSize);
	for (int i = 0; i < vector.size(); i++) {
		vector[i] = data_buffer_[i];
	}
}

void PumaRedisDriver::eigenVectorToBuffer(const Eigen::VectorXd& vector) {
	assert(vector.size() < kDof || vector.size() < kTaskSize);
	for (int i = 0; i < vector.size(); i++) {
...
void PumaRedisDriver::run() {
	while (g_runloop) {
		...
		// Parse control mode
		auto mode_keyval = kControlModeMap.find(mode_str);
		if (mode_keyval == kControlModeMap.end()) {
			stop();
		...
		switch (control_mode_) {
			...
			case NJTRACK: case JTRACK:
				if (command_data_.size() != kDof) {
					stop();
					std::cerr << "ERROR: Redis key (" << KEY_COMMAND_DATA << ", "
					         << command_data_.transpose() << ") must be of length "
					         << kDof << " for control mode_ " << control_mode_ << "."
					         << std::endl;
					continue;
				}
				break;
			...
			case NTRACK:  case TRACK:
				if (command_data_.size() != kTaskSize) {
					stop();
					std::cerr << "ERROR: Redis key (" << KEY_COMMAND_DATA << ", "
					         << command_data_.transpose() << ") must be of length "
					         << kTaskSize << " for control mode_ " << control_mode_ << "."
					         << std::endl;
					continue;
				}
				break;
			case PFMOVE:
			case LINE:
				// TODO: Figure out proper size for PFMOVE and LINE
			default:
				break;
		}
		...
		if (kp_str_new != kp_str_) {
			...
			if (kp_.size() != kDof) {
				...
				std::cerr << "ERROR: Redis key (" << KEY_KP << ", "
			         	 << kp_.transpose() << ") must be of length " << kDof
			         	 << "." << std::endl;
				break;
			} else if (isnan(kp_)) {
				stop();
		...
		if (kv_str_new != kv_str_) {
			...
			if (kv_.size() != kDof) {
				...
				std::cerr << "ERROR: Redis key (" << KEY_KV << ", "
			         	 << kv_.transpose() << ") must be of length " << kDof
			         	 << "." << std::endl;
				break;
			} else if (isnan(kv_)) {
				stop();
