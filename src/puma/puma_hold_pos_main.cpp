...
const std::string kWorldFile = "resources/puma_hold_pos/world.urdf";
const std::string kRobotFile = "resources/puma_hold_pos/kuka_iiwa.urdf";
const std::string kRobotName = "Kuka-IIWA";

const double arr_kp_joint[Puma::DOF] = {100, 100, 60, 60, 10, 8, 5};
const double arr_kv_joint[Puma::DOF] = {20, 20, 10, 10, 5, 3, 1};
...
int main() {
	...
	// Read from Redis
	robot->_q = redis.getEigenMatrix(Puma::KEY_JOINT_POSITIONS);
	robot->_dq = redis.getEigenMatrix(Puma::KEY_JOINT_VELOCITIES);

	// Update the model
	robot->updateModel();

	// Initialize controller variables
	Eigen::VectorXd command_torques = Eigen::VectorXd::Zero(Puma::DOF);
	Eigen::VectorXd q_initial = robot->_q;

	Eigen::DiagonalMatrix<double, Puma::DOF> kp_joint;
	kp_joint.diagonal() << 100, 100, 60, 60, 10, 8, 5;
	Eigen::DiagonalMatrix<double, Puma::DOF> kv_joint;
	...
	while (g_runloop) {
		...
		redis.setEigenMatrix(Puma::KEY_COMMAND_TORQUES, command_torques);
